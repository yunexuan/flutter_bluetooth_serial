...
public class FlutterBluetoothSerialPlugin implements FlutterPlugin, ActivityAware {
    ...
    private ConcurrentHashMap<Integer, ReadThread> readThreadConcurrentHashMap = new ConcurrentHashMap<>(1);
...
//                        byte[] bytes2 = hexStringToByteArray(byteArrayToHexString(byte1));
                ...
                case "byteArrayToHexString" :{
                    byte[] data = call.argument("data");
                    result.success(byteArrayToHexString(data));
                    break ;
                }
                case "close" :{
                    Log.d(TAG, "蓝牙关闭成功！");
...
//                    byte[] bytes1 = HexString2Bytes("AA558100000106BB66");
    ...
    public byte[] getPirntMessage(byte[] printbytes) {
    ...
    public static byte[] combineArrays(byte[]... info) {
        int length = 0;
        for (byte[] arr : info) {
            length += arr.length;
        }
    ...
    private class ReadThread extends Thread{
        ...
        public ReadThread(BluetoothConnection connection) {
            ...
            while (isLoop) {
                try {
                    if(connection.isConnected()){
                        byte[] bytes1 = HexString2Bytes("AA550403BB66");
                        connection.write(bytes1);
                        Thread.sleep(250);
                    }
                } catch (Exception e) {
